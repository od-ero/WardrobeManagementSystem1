...
import Edit, { Destroy, Preview, Restore } from '@/app/(app)/roles-permissions/modals'
...
const MyListingPage = ({ page_id }) => {
    ...
    const { data:organizations, data_mutate, loading: activeLoading } = list(filter)


    const active_organization = 'Active Roles'
    const inactive_organization = 'Inactive Roles'
    ...
    const CustomdropDown = ({ row }) => (
        ...
        >
            ...
            <DropdownLink href={`/roles-permissions/${row.id}`}>View</DropdownLink>
